...
import no.sandramoen.transagentx.actors.ShockwaveBackground
...
class SkeiveSpillutviklereSplashScreen : BaseScreen() {
    private lateinit var shockwaveBackground: ShockwaveBackground
    private lateinit var blackOverlay: BaseActor

    override fun initialize() {
        shockwaveBackground = ShockwaveBackground("images/excluded/Light.png", uiStage)
    ...
    private fun disposeAndSetActiveScreen() {
        blackOverlay.addAction(Actions.after(Actions.run {
            dispose()
            BaseGame.setActiveScreen(MenuScreen(playMusic = true))
        }))
    }
}
